...
const nav = [
  ...
  ['status','Status'],['drift','Drift'],['policy','Policy'],['roles','Roles'],['skills','Skills'],
  ['sync','Managed Sync'],['github-action','GitHub Action'],['mcp','MCP'],['sdk','SDK'],
  ['provider-surfaces','Provider Surfaces'],['trust-flow','Trust Flow'],
  ['dashboard','Dashboard'],['diagnostics','Diagnostics'],['vscode','VS Code'],['security','Security'],
  ['benchmarks','Benchmarks'],['migration','Migration'],
  ['agents-md-validator','AGENTS.md Validator'],['ai-agent-readiness','AI Agent Readiness'],
  ['context-optimization','Context Optimization'],['mcp-repository-tools','MCP Repository Tools']
];
...
function pages(version) {
  ...
  return [
    {slug:'',title:'AgentContextKit',description:'Agent readiness and repository context toolkit for Codex, Claude Code, Cursor, Copilot, Gemini and MCP-based coding agents. Offline-first, deterministic.',body:`<h1>AgentContextKit ${esc(version)}</h1><p><strong>Turn repository context into something agents and humans can inspect.</strong> ACKit combines readiness scoring, instruction graphs, context packs, policy-as-code, security scanning, tasks, MCP, dashboard and editor tooling without sending repository data to a hosted analysis service.</p><p>Agent context and repository readiness for <strong>Codex, Claude Code, Cursor, Copilot and Gemini</strong> — validate agent instructions, scan repositories, optimize context and enforce evidence-backed workflows. Offline-first, deterministic, no telemetry.</p><p><a class="button" href="/agent-context-kit/getting-started/">Getting Started</a><a class="button" href="/agent-context-kit/cli/">CLI Reference</a><a class="button" href="${REPO}">GitHub</a><a class="button" href="${NPM}">npm</a></p><p>Try it without installing: <code>npx --yes @cynrath/agent-context-kit@latest readiness</code> — one command scores any repository 0–100 with actionable findings.</p><h2>Install</h2><pre><code>${esc(install)}</code></pre><h2>Quickstart</h2><pre><code>ackit init --dry-run\nackit readiness\nackit instructions --explain\nackit scan --ci\nackit sync --check\nackit pack --profile codex --max-tokens 50000\nackit dashboard --port 0 --open</code></pre><h2>Core capabilities</h2><ul><li><strong>Agent Readiness</strong> — explainable 0–100 scoring with strict gates and baselines.</li><li><strong>Instruction Graph v2</strong> — provider-aware scope, precedence, provenance, conflicts and duplicates.</li><li><strong>Context Packs</strong> — deterministic ranking, token budgets and per-file manifests.</li><li><strong>Policy Packs</strong> — machine-checkable repository rules.</li><li><strong>Scanning + SARIF</strong> — secrets, connection strings, entropy, absolute paths, CI pinning and redacted evidence.</li><li><strong>Tasks</strong> — docs-first single-active workflow and completion gates.</li><li><strong>Workflows + Intent</strong> — staged lifecycles with gates, committed intent docs with fingerprints.</li><li><strong>Evidence + State-Bound Verification</strong> — criteria-to-proof registry, state-bound bundles (<code>ackit.verification-bundle.v2</code>), persisted bound verdicts (<code>ackit.verdict.v2</code>) with verifier independence and replay/staleness refusal.</li><li><strong>Checkpoints + Portable Handoff v2</strong> — snapshots, verification-bound portable handoff (<code>ackit.handoff.v2</code>), read-only import with stale refusal.</li><li><strong>Canonical Status</strong> — read-only task/stage, blockers, verification and checkpoint freshness, next actions (<code>ackit.status.v1</code>).</li><li><strong>Provider / Surface Parity</strong> — CLI, SDK, read-only MCP, Action and VS Code project the same status snapshot.</li><li><strong>Containment Hardening</strong> — link-aware output guard with symlink/junction escape repair; MCP stays read-only.</li><li><strong>Trust-Flow Demo</strong> — reproducible offline proof from missing evidence to second-process resume.</li><li><strong>Drift + Policy v2</strong> — deterministic drift findings, risk-tiered autonomy and review policy.</li><li><strong>Roles + Skills</strong> — portable role contracts, managed open-standard skills.</li><li><strong>Managed Sync</strong> — content-driven reconciliation of ACKit-owned assets after upgrades.</li><li><strong>MCP + SDK</strong> — local integrations for agents and tooling.</li><li><strong>Dashboard + VS Code</strong> — local inspection surfaces for readiness, findings, graph, tasks and policy.</li></ul>`},
    ...
    {slug:'checkpoints',title:'Checkpoints',description:'Resumable checkpoints with verification-bound portable handoff.',body:`<h1>Checkpoints / Resume / Handoff</h1><p>Checkpoints snapshot task state with deterministic resume context and handoff packs, so work survives provider switches and interruptions.</p><pre><code>ackit checkpoint create --next-objective "wire gate" --next-path src/core --next-command "pnpm test" --next-expected "green"\nackit checkpoint show\nackit checkpoint validate\nackit checkpoint export --format json --out .ackit/reviews/handoff.json\nackit checkpoint import .ackit/reviews/handoff.json\nackit task resume</code></pre><h2>Portable handoff v2</h2><p><code>ackit.handoff.v2</code> binds task and workflow state, the checkpoint record, evidence presence, the verification binding (state and bundle digests) with the latest verdict trust summary, checkpoint staleness at export time, the canonical status snapshot (verbatim blockers plus derived next actions), a redaction manifest, and provider-neutral resume instructions. Imports validate against current disk state and stay read-only: fresh handoffs render resume context, moved state refuses with <code>VERDICT-STATE-STALE</code>, v1 markdown refuses with <code>HANDOFF-V1-UNBOUND</code>, malformed files with <code>HANDOFF-INVALID</code>, and unknown tasks with <code>HANDOFF-TASK-UNKNOWN</code>.</p>`},
    ...
    {slug:'verification',title:'Verification',description:'State-bound verification bundles and independent verdicts.',body:`<h1>Verification / Verdicts</h1><p>Verification bundles (<code>ackit.verification-bundle.v2</code>) give a fresh independent verifier everything needed to judge a task, bound to the exact state via <code>stateDigest</code> and <code>bundleDigest</code>. The verifier authors <code>ackit.verdict.v1</code>; ACKit persists a state-bound <code>ackit.verdict.v2</code>. Legacy v1 files stay readable as history but never satisfy a state-bound completion.</p><pre><code>ackit verification bundle --format json --out .ackit/reviews/bundle.json\nackit verification record --verdict ./verdict.yaml --bundle .ackit/reviews/bundle.json\nackit verification show</code></pre><h2>Independence and freshness</h2><p>Independence is proven by <code>reviewedBundleDigest</code>: fresh claims without bundle proof refuse with <code>VERDICT-INDEPENDENCE-UNPROVEN</code>, replayed content refuses with <code>VERDICT-REPLAY-REJECTED</code>, stale bundles with <code>VERDICT-BUNDLE-MISMATCH</code>, and moved state with <code>VERDICT-STATE-STALE</code>. Completion rechecks freshness; no cryptographic verifier identity is claimed.</p>`},
    {slug:'status',title:'Status',description:'Canonical read-only task status with blockers and next actions.',body:`<h1>Status</h1><p><code>ackit status [id]</code> is the canonical read-only projection: task and stage, verbatim completion blockers, verification and checkpoint freshness, and derived next actions. Human output and stable <code>ackit.status.v1</code> JSON describe the same snapshot; the command never mutates.</p><pre><code>ackit status\nackit status TASK-0007\nackit --json status TASK-0007</code></pre>`},
    ...
    {slug:'mcp',title:'MCP',description:'Local Model Context Protocol integration for ACKit.',body:`<h1>MCP</h1><p>ACKit exposes a local stdio MCP server for read-oriented repository inspection (16 read-only tools including canonical <code>ackit_status</code>). No remote transport is required.</p><pre><code>{ "mcpServers": { "ackit": { "command": "ackit", "args": ["mcp", "serve"] } } }</code></pre>`},
    {slug:'sdk',title:'SDK',description:'Programmatic ACKit repository analysis.',body:`<h1>SDK</h1><p>The ESM SDK exposes the same deterministic repository analysis primitives used by the CLI.</p><pre><code>import { scanRepository } from "@cynrath/agent-context-kit";\nconst result = await scanRepository({ canonicalPath: process.cwd() });</code></pre><p>Read-model parity: <code>buildStatusReport</code> and <code>renderStatusReport</code> project the same <code>ackit.status.v1</code> snapshot as <code>ackit status</code>; handoff and independence exports share the CLI contract.</p>`},
    {slug:'provider-surfaces',title:'Provider Surfaces',description:'Canonical provider-surface mapping with CLI, SDK, MCP, Action and VS Code parity.',body:`<h1>Provider Surfaces</h1><p>ACKit projects one canonical workflow, evidence, verdict, and status snapshot across every integration surface. Primary-source dates and caveats live with the product docs.</p><ul><li><strong>Codex CLI</strong> — AGENTS.md-native instructions with nested scope.</li><li><strong>Claude Code</strong> — CLAUDE.md-native instructions with rules paths.</li><li><strong>Gemini CLI</strong> — GEMINI.md instructions with JIT discovery.</li><li><strong>Copilot Chat</strong> — copilot-instructions plus AGENTS.md with applyTo scopes.</li><li><strong>Copilot coding agent</strong> — multi-native AGENTS plus CLAUDE plus GEMINI.</li><li><strong>Copilot review</strong> — AGENTS-only channel with repo toggle and HEAD sourcing.</li><li><strong>VS Code agent mode</strong> — settings-gated multi-native instructions.</li></ul><p>CLI (<code>ackit status</code>), SDK (<code>buildStatusReport</code>), read-only MCP (<code>ackit_status</code>), the GitHub Action (<code>command: status</code>), and the VS Code Tasks view all expose the same snapshot.</p>`},
    {slug:'trust-flow',title:'Trust Flow',description:'Reproducible verification-bound trust flow from evidence to resume.',body:`<h1>Trust Flow</h1><p>One reproducible offline arc: missing evidence and verdicts block completion, a bound independent verdict enables it, a state mutation stales it, fresh re-verification restores it, a checkpoint handoff exports it, and a second process resumes it with zero shared memory.</p><pre><code>ackit task complete TASK-0007\nackit verification bundle TASK-0007 --format json --out .ackit/reviews/bundle.json\nackit verification record TASK-0007 --verdict .ackit/reviews/verdict.yaml --bundle .ackit/reviews/bundle.json\nackit status TASK-0007\nackit checkpoint export TASK-0007 --format json --out .ackit/reviews/handoff.json\nackit checkpoint import .ackit/reviews/handoff.json</code></pre><p>Executed by <code>tests/e2e/trust-flow-demo.test.ts</code> and walked through in the product guide <code>docs/guides/demo-trust-flow.md</code>.</p>`},
    ...
    {slug:'vscode',title:'VS Code',description:'AgentContextKit VS Code extension.',body:`<h1>VS Code</h1><p><code>Cynrath.ackit-vscode</code> surfaces readiness, findings, instructions, tasks, policy and diagnostics inside VS Code. The Tasks view renders the canonical status snapshot (blockers plus next actions) with no write paths added.</p><pre><code>code --install-extension Cynrath.ackit-vscode</code></pre>`},
    {slug:'security',title:'Security',description:'ACKit offline-first threat model and redaction guarantees.',body:`<h1>Security</h1><p><strong>Offline-first:</strong> product analysis is designed for zero outbound repository-data egress after installation.</p><ul><li>No telemetry or hosted LLM calls in repository analysis.</li><li>Evidence is redacted at construction.</li><li>Path containment and traversal checks protect file access.</li><li>Rule evaluation includes ReDoS and size guards.</li><li>Dashboard defaults to loopback with CSP.</li><li>v0.5 containment delta: link-aware output guard with symlink/junction escape audit and proven-gap-only repair; MCP remains read-only.</li></ul>`},
    ...
    {slug:'agents-md-validator',title:'AGENTS.md Validator',description:'Validate AGENTS.md and CLAUDE.md instructions; find conflicts, duplicates and scope problems.',body:`<h1>AGENTS.md Validator</h1><p>One repository, many agents: <code>AGENTS.md</code> (Codex), <code>CLAUDE.md</code> (Claude Code), <code>GEMINI.md</code> (Gemini) and Copilot instructions plus nested scopes. When files disagree, agents follow stale or contradictory guidance and waste runs.</p><p>ACKit resolves every instruction file into a provider-aware graph with scope, precedence and provenance, then reports conflicts, duplicates, shadowing and dead files — before your agent reads them.</p><pre><code>ackit instructions --explain\nackit optimize --explain</code></pre><pre><code>codex/AGENTS.md → claude/CLAUDE.md (provenance: frontmatter applyTo)</code></pre><p>Try it without installing: <code>npx --yes @cynrath/agent-context-kit@latest readiness</code></p><p><a class="button" href="/agent-context-kit/instruction-graph/">Instruction Graph</a><a class="button" href="/agent-context-kit/optimize/">Optimize</a><a class="button" href="${REPO}">GitHub</a><a class="button" href="${NPM}">npm</a></p>`},
    {slug:'ai-agent-readiness',title:'AI Agent Readiness',description:'Score any repository 0–100 for AI coding-agent readiness with explainable deductions.',body:`<h1>AI Agent Readiness</h1><p>Is your repository actually ready for Codex, Claude Code, Cursor, Copilot or Gemini — or will the agent burn context on missing instructions, leaked secrets and bloated includes? Guessing costs tokens; measuring is one command.</p><p>ACKit scores instructions, security, context efficiency, task hygiene, skills and policy on a deterministic 0–100 scale with weighted renormalization, strict gates and baselines.</p><pre><code>ackit readiness\nackit readiness --strict\nackit readiness --fail-below 80</code></pre><pre><code>Readiness  89/100  ██████████████████░░  (threshold 0 — pass)\n  Instructions          90/100\n  Security              90/100\n  Context Efficiency    70/100</code></pre><p>Try it without installing: <code>npx --yes @cynrath/agent-context-kit@latest readiness</code></p><p><a class="button" href="/agent-context-kit/readiness/">Readiness</a><a class="button" href="/agent-context-kit/cli/">CLI Reference</a><a class="button" href="${REPO}">GitHub</a><a class="button" href="${NPM}">npm</a></p>`},
    {slug:'context-optimization',title:'Context Optimization',description:'Cut wasted agent context: duplicates, oversized files and low-value includes.',body:`<h1>Context Optimization</h1><p>Every duplicated guidance block and oversized context file is tokens your agent pays for on every run. ACKit finds the waste with evidence — file paths, token-waste estimates and provenance — and proposes safe, reviewable fixes.</p><p>Pair the advisor with budgeted, provider-aware context packs so each agent gets exactly the context it needs.</p><pre><code>ackit optimize --explain\nackit pack --profile codex --max-tokens 50000</code></pre><pre><code># Optimize Report (1 suggestion(s))\n- **[low] budget-overrun** (low): 867 file(s) did not fit the 20000-token budget</code></pre><p>Try it without installing: <code>npx --yes @cynrath/agent-context-kit@latest readiness</code></p><p><a class="button" href="/agent-context-kit/optimize/">Optimize</a><a class="button" href="/agent-context-kit/profiles/">Provider Profiles</a><a class="button" href="${REPO}">GitHub</a><a class="button" href="${NPM}">npm</a></p>`},
    {slug:'mcp-repository-tools',title:'MCP Repository Tools',description:'Expose repository context to coding agents over local MCP stdio — read-only.',body:`<h1>MCP Repository Tools</h1><p>Coding agents that speak the Model Context Protocol need repository context: readiness, findings, instruction graphs, tasks and policy. Pointing them at raw files loses precedence, provenance and redaction.</p><p>ACKit serves the same deterministic analysis over local stdio MCP — 16 read-only tools including the canonical status snapshot — with no network transport and no write paths.</p><pre><code>{ "mcpServers": { "ackit": { "command": "ackit", "args": ["mcp", "serve"] } } }</code></pre><p>Try it without installing: <code>npx --yes @cynrath/agent-context-kit@latest readiness</code></p><p><a class="button" href="/agent-context-kit/mcp/">MCP</a><a class="button" href="/agent-context-kit/provider-surfaces/">Provider Surfaces</a><a class="button" href="${REPO}">GitHub</a><a class="button" href="${NPM}">npm</a></p>`}
  ];
}
